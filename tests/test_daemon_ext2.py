...
class TestDaemonPid:
    def test_write_pid(self, tmp_path):
        ...
        c = DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log"))
        Daemon(c)._write_pid()
    ...
    def test_get_pid(self, tmp_path):
        ...
        assert Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log")))._get_pid() == 99999

    def test_get_pid_none(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        assert Daemon(DaemonConfig(pid_file=str(tmp_path / "x.pid"), log_file=str(tmp_path / "t.log")))._get_pid() is None


class TestDaemonRunning:
    def test_not_running_no_pid(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        assert Daemon(DaemonConfig(pid_file=str(tmp_path / "x.pid"), log_file=str(tmp_path / "t.log"))).is_running() is False

    def test_is_running(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        (tmp_path / "p.pid").write_text(str(os.getpid()))
        assert Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log"))).is_running() is True

    def test_stale_pid(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        (tmp_path / "p.pid").write_text("999999999")
        assert Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log"))).is_running() is False
...
class TestDaemonStats:
    ...
    def test_load_missing(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        d = Daemon(DaemonConfig(stats_file=str(tmp_path / "m.json"), log_file=str(tmp_path / "t.log"), pid_file=str(tmp_path / "p.pid")))
...
class TestDaemonSignals:
    def test_handle_shutdown(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        d = Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log")))
        d._running = True
...
class TestHealthChecker:
    def test_init(self, tmp_path):
        from daemon import HealthChecker, DaemonConfig
        c = HealthChecker(DaemonConfig(log_file=str(tmp_path / "t.log"), pid_file=str(tmp_path / "t.pid")))
        assert c.checks == []
    ...
    def test_check_all_healthy(self, tmp_path):
        from daemon import HealthChecker, DaemonConfig, DaemonState
        c = HealthChecker(DaemonConfig(log_file=str(tmp_path / "t.log"), pid_file=str(tmp_path / "t.pid")))
        c.register_check("ok", lambda: True)
...
class TestDaemonStart:
    ...
    def test_signal_handlers(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        d = Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log")))
        with patch.object(d, "_main_loop"), patch("os.chdir"), patch("os.setsid"), patch("signal.signal") as ms:
    ...
    def test_run_on_start_called(self, tmp_path):
        from daemon import Daemon, DaemonConfig
        cb = MagicMock()
        d = Daemon(DaemonConfig(pid_file=str(tmp_path / "p.pid"), log_file=str(tmp_path / "t.log"), run_on_start=cb))
        with patch.object(d, "_main_loop"), patch("os.chdir"), patch("os.setsid"):
            d.start(daemonize=False)
